...
vector <Mat> generateGaborFiltersCPU()
{
	...
	//My code Gabor Coefficients calculation
	...
	/*for (int n=0; n<numOfFilters; n++)
	{
		...
		for (int j=0; j<filterSizeY; j++)
			for (int i=0; i<filterSizeX; i++)
			{
				int indx = j*filterSizeX + i;
				tr[indx]=t[indx]+gaborParams[n][3]; 
				if(tr[indx] < -PI) 
                    tr[indx] += 2.0f*PI;
                else if (tr[indx] > PI) 
                    tr[indx] -= 2.0f*PI;
			
				G_data[indx] = exp(-10.0f*gaborParams[n][0]*(fr[indx]/filterSizeY/gaborParams[n][1]-1)*
					(fr[indx]/filterSizeX/gaborParams[n][1]-1)-2.0f*gaborParams[n][2]*PI*tr[indx]*tr[indx]);
			}
		G.push_back(G0);
	}*/
