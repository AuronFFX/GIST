...
cudaEventRecord(start);
	       ...
	       /* for(int j = 0; j < height; j++)
	        {
	            ...
	            {
	                buffer[(j*width+i)*2+0] = buffer[(j*width+i)*2+0] * G[k*width*height+j*width+i];
	                buffer[(j*width+i)*2+1] = buffer[(j*width+i)*2+1] * G[k*width*height+j*width+i];
	            }
	        }*/
		...
		//if (1) cout << "Full calcGist time: " << elapsed_time << endl;
